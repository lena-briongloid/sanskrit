function gramwiz_suggest(num) {
	...
	for (var i = 0; i < SugText.length; i ++) {
...
function gramwiz_result() {
	...
	if (["m", "f", "n"].includes(grid(dict, column.class, gramwizIndex[1]))) {
		...
		source_sandhi += `<p><strong><em>${sandhi_iast}</em></strong>&nbsp;<small class="IPA">[${sandhi_pron}]</small></p>`;

		if (properties.showDevanagari) {
			source_sandhi += `<p class="sanskrit" style="font-size: 32px;">${LattoDev(sandhi_iast)}</p>`;
		}
	...
	//case 2: b is adj
		...
		let source_sandhi = "";
		let text_gender = ["(남성형)", "(여성형)", "(중성형)"];

		let a_comp = grid(dict, column.grammar, gramwizIndex[0]);

		for (var i = 1; i <= 3; i ++) {
			let b_sole = grid(dict, column.grammar + i, gramwizIndex[1]);

			let sandhi_iast = gramwiz_sandhi(a_comp, b_sole);
			let sandhi_pron = showPronounce(sandhi_iast, true);

			source_sandhi += `<p><span style="font-size: 24px;">${text_gender[i - 1]}</span>&nbsp;<strong><em>${sandhi_iast}</em></strong>&nbsp;<small class="IPA">[${sandhi_pron}]</small></p>`;
